<VirtualHost *:443>
  ...
  <FilesMatch ".php$">
         SetHandler "proxy:unix:/etc/endurance/configs/php/{{$current_php_version}}/{{ $domain_without_www }}.sock|fcgi://localhost/"
   </FilesMatch>

  SSLEngine on
  SSLCertificateFile {{ $ssldir}}ca.cer
  SSLCertificateKeyFile {{ $ssldir}}{{$domain_without_www}}.key
  SSLCertificateChainFile {{ $ssldir}}fullchain.cer
